...
import Adafruit_DHT                                     #Import the function with reads the temperature and humidity of DHT11 and DHT22
...
humiditylist = []                                       #Creating the list Humidity
templist = []                                           #Creating the list Temperature
Date = []                                               #Creating the list Date
Time = []                                               #Creating the list Time 
...
class HomeScreen(gui.Desktop):                          #Create the container of the HomeScreen and add all the buttons and tables
    def __init__(self,**params):
        ...
        select.add("DHT11",Adafruit_DHT.DHT11)                      #Sensors that can be choosen
    ...
    def action_new(self,value):                                     #Setting what the actions New, Save, Saveas and Open do
        self.new_d.close()
...
def WriteTableHomeScreen():                                         #Function that Reads the sensor and prints in the table
    ...
    while flagStart:
        humidity, temp = Adafruit_DHT.read_retry(select.value,2)    #Read the sensor
        print('humidity{0:0.1f} temp{0:0.1f}'.format(humidity, temp))
        humiditylist.append(humidity)                               #Add to the humidity and temperature lists
        templist.append(temp)
        DateTime = time.localtime()                                 #Reads the time and add in the date and time lists
        ...
        DateTime = ChangeMonthtoPortuguese(DateTime)                #Just change to portuguese to print in the screen
